...
describe("authentication tests", () => {
  ...
  const requestLoginUser = () => request(app).post(routeInAuthRouter("/login"));
  ...
  describe("login", () => {
    ...
    test("login should return different tokens for each login", async () => {
      ...
      const { token: firstLoginAccesToken } = firstLogin.body;
      const { token: secondLoginAccesToken } = secondLogin.body;
  ...
  describe("logout", () => {
    test(
      ...
      async () => {
        ...
        const { userId } = loginResponse.body;
  ...
  describe("refresh token", () => {
    ...
    test("refresh should insert new refresh token to user", async () => {
      const loginResponse = await loginUser(testUserLogin);
      const { userId } = loginResponse.body;
      const refreshToken = extractRefreshTokenFromResponseHeader(loginResponse);
    ...
    test("refresh expired token should return valid token", async () => {
      ...
      const createPostResponse = await requestAuthenticatedRoute(token);
      ...
      const createPostResponseWithValidToken = await requestAuthenticatedRoute(
        newAccessToken
      );
      expect(createPostResponseWithValidToken.status).toBe(StatusCodes.OK);
    }, 10_000);
  });
});
